...
const total = { "freezing temps": 10, "clear skies": -2 }
function totalLicks(env) {
  // The Owl says, "Code Me!"
  const normLicks = 252;
  const allLicks = Object.values(env);
  const challenge = Object.keys(env);

  const totalLicks =  allLicks.length > 0 ? allLicks.reduce((acc, num) => acc + num) : 0;

  const tough = allLicks.findIndex(el => el === Math.max(...allLicks) && el > 0);
  return tough >= 0 ?
          `It took ${normLicks + totalLicks} licks to get to the tootsie roll center of a tootsie pop. The toughest challenge was ${challenge[tough]}.` :
          `It took ${normLicks + totalLicks} licks to get to the tootsie roll center of a tootsie pop.`

}
console.log(totalLicks(total))
