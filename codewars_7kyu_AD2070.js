...
const exData =  [['Distance:', 116.28, 'Meters', 'Sensor 5 malfunction =>lorimar'], ['Distance:', 117.1, 'Meters', 'Sensor 5 malfunction =>lorimar'], ['Distance:', 123.96, 'Meters', 'Sensor 5 malfunction =>lorimar'], ['Distance:', 117.17, 'Meters', 'Sensor 5 malfunction =>lorimar']];
function sensorAnalysis(sensor_data){
  //your code here
  const averageData = sensor_data.reduce((acc,data)=> {
    const distance = data[1];
    acc += distance;
    return acc;
  },0) / sensor_data.length;

  const deviation = Math.sqrt(sensor_data.reduce((acc, data) => {
    const distance = data[1];
    acc += Math.pow(distance - averageData, 2);

    return acc
  }, 0) / (sensor_data.length - 1));

  return [parseFloat(averageData.toFixed(4)), parseFloat(deviation.toFixed(4))]
}
console.log(sensorAnalysis(exData))
